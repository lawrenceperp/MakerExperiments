...
import { OpenOrder, AmountType, Side } from "@perp/common/build/lib/perp/PerpService"
...
interface Market {
    ...
    // maker
    liquidityAmount: Big
    liquidityRangeMultiplierLower: Big
    liquidityRangeMultiplierUpper: Big
...
export class Maker extends BotService {
    ...
    async createMarketMap() {
        ...
        for (const [marketName, market] of Object.entries(config.MARKET_MAP)) {
            ...
            this.marketMap[marketName] = {
                ...
                liquidityRangeMultiplierLower: Big(market.LIQUIDITY_RANGE_OFFSET_LOWER),
                liquidityRangeMultiplierUpper: Big(market.LIQUIDITY_RANGE_OFFSET_UPPER),
    ...
    async createOrder(market: Market): Promise<OpenOrder> {
        ...
        const marketPrice = await this.perpService.getMarketPrice(market.poolAddr)
        const upperPrice = marketPrice.mul(market.liquidityRangeMultiplierUpper)
        const lowerPrice = marketPrice.mul(market.liquidityRangeMultiplierLower)
        ...
        await this.openPosition(this.wallet,market.baseToken, Side.SHORT, AmountType.BASE, Big(liquidityAmount.div(2).div(marketPrice)),undefined,undefined,this.referralCode);
